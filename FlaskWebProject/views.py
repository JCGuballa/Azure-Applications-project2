...
def authorized():
    ...
    if request.args.get('code'):
        cache = _load_cache()
        # To Acquire a token from a built msal app, along with the appropriate redirect URI
...
def _load_cache():
    # To Load the cache from `msal`, if it exists
...
def _save_cache(cache):
    # To Save the cache, if it has changed
    if cache.has_state_changed:
        session['token_cache'] = cache.serialize()

def _build_msal_app(cache=None, authority=None):
    # To Return a ConfidentialClientApplication
...
def _build_auth_url(authority=None, scopes=None, state=None):
    # To Return the full Auth Request URL with appropriate Redirect URI
